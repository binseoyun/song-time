...
## 9. 벤치마크 재작성 이력 (#111, 2026-08-31)

`seedData.js`가 #89(PR #99)에서 소프트웨어학부 2026-2 실데이터(**22과목 / 37분반**)로
교체되면서, 기존 `questions.yaml`(74 시나리오)이 참조하던 과목 상당수가 시드에서
사라졌다(클라우드시스템·인공지능입문·프로그래밍방법론·데이터마이닝및분석·독일어Ⅰ·
공학수학I·알고리즘입문·웹프로그래밍(캡스톤디자인) 등). `department`도 컴퓨터공학 →
소프트웨어학부로 바뀌어 "소프트웨어" keyword가 학과명에 걸리는 등 매칭 양상이 달라졌다.

#111에서 다음을 새 시드 실측값으로 전면 재작성했다. **시나리오 수(74)·카테고리 분포·
각 시나리오가 노리는 프로브 성격은 유지**했다.

- `backend/ai-server/eval/questions.yaml` — 헤더 참조 블록 + 74 시나리오 (turn 86개)
- `backend/scripts/seedBenchmarkSeats.js` — 잔여석 픽스처를 새 과목 코드로 교체.
  값은 `capacity - enrolled`(구 챗봇이 잘못 쓰던 소스)와 다르게 잡아 Before/After 변별력 유지,
  분반비교 note의 "자리 제일 많은 분반" 결론 정합. 관심 등록(enrolled)이 정원 만석인
  과목(딥러닝개론·소프트웨어공학·인공지능산업체특강)에 여유 좌석을 심어 "구 방식은 마감이라
  답한다"가 드러나게 함.
- `backend/ai-server/eval/rag_questions.yaml` 라우팅 섹션은 새 시드와 정합함을 확인(변경 없음).

**baseline(gemini-3.6-flash, 2026-08-26, 구 92과목 세트) 대비 이번 측정은 다른 시드·
다른 문항이다.** Before/After 표에서는 이 점을 각주로 남기고, 옛 모델을 재실행하지는 않는다
(baseline 수치는 `03-결과.md`에 저장돼 있음).
