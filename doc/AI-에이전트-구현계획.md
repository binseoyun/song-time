...
## Stage 2 — write Tool + 스트리밍 + UI (~4~6일)

목표: 실제로 신청까지 도와주는 에이전트로 완성한다. ADR-010 §3/§9/§11 반영.

- [ ] 2-1. **선행 확인**: 로드맵 Phase 2 "최종 개선안 선택 + ADR 작성"(courseController.js 최종 등록 API 확정) 완료 여부 확인 — 미완료면 이 Stage 착수 전에 먼저 마무리하거나, 최소한 write Tool이 호출할 API를 그 시점 기준으로 재확인
- [ ] 2-2. write Tool(신청/취소) 연동 + LLM 파라미터 제안 → 프론트 확인 카드 → 사용자 승인 후 실행 흐름 (ADR-010 §9)
- [ ] 2-3. `POST /api/ai/chat`을 SSE 스트리밍으로 전환 + nginx `proxy_buffering off`/타임아웃 설정 추가 (ADR-010 §11)
- [ ] 2-4. 프론트 채팅 UI(기존 "실시간 수강신청 연습" 탭과 자연스럽게 연결되는 위치 검토)
