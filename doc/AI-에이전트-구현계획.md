...
- [x] 3-0. 챗봇 사용자 단위 rate limit (이슈 #108). **§15 재검토 — Node 계층 → `ai-server` 계층** (완전 MSA: 챗봇 비용은 챗봇 서비스 책임, `redis-chat` 소유). `chat/rate_limit.py` — `POST /api/ai/chat` 진입부에서 `redis-chat` 카운터(`chat:ratelimit:{uid}`), `INCR`+`EXPIRE` Lua 원자화, 초과 시 429 + `Retry-After`, Redis 장애 fail-open. Node `aiController`가 429 파싱해 전달. `chat/test_rate_limit.py` 4개 + E2E(nginx→Node→ai-server) 429 확인. 값 `CHAT_RATE_LIMIT=20`/`WINDOW=60`(placeholder, 3-3에서 확정). 향후 API Gateway 도입 시 거기에 coarse 계층 추가(defense in depth).
...
- [x] 3-2. 챗봇 장애 폴백 (이슈 #106). `chat/errors.py` — `classify_llm_error` 3종(transient/quota/unknown), raw 예외는 서버 로그에만·SSE엔 분류 메시지만. `build_llm(max_retries=2)`(429/503만). `_persist_turn`으로 실패 시에도 사용자 메시지 저장 + 부분 답변 `[응답이 중간에 끊겼습니다]` 꼬리표. Tool(과목 API·Qdrant·임베딩) 장애는 `{"error": ...}` 반환 → 에이전트가 안내. 수동 장애 주입 3종 검증(Qdrant/backend/키 오류). → [리팩토링 03](refactoring/03-챗봇-장애-폴백.md)
