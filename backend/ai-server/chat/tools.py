...
def search_courses(keyword: str = "") -> List[Dict[str, Any]]:
    """과목명·학과명·담당 교수명에 특정 단어가 든 과목들을 빠짐없이 조회한다. "리눅스 들어가는
    수업", "네트워크 관련 강의 목록", "모바일 과목 있어?"처럼 한 단어로 과목을 나열해 달라는
    요청은 이 Tool을 쓴다(강의에서 다루는 세부 개념·기법으로 찾을 때만 search_syllabus).
    keyword에는 핵심어만 넣는다 — "교수님"·"관련"·"수업" 같은 군더더기나 조사는 빼고 교수명이나
    과목명 조각만 (예: "창병모", "데이터베이스", "리눅스"). 반환값에는 각 과목의 담당 교수·학점·정원과 함께
