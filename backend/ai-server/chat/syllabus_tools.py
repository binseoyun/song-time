...
def search_syllabus(query: str) -> list[dict[str, Any]]:
    """강의에서 배우는 세부 개념·기법을 여러 개 나열해 그걸 다루는 과목을 찾을 때만 쓴다.
    예: "동적 계획법이랑 그리디 배우는 수업", "트랜스포머랑 멀티모달 이론 다루는 과목",
    "쉘 스크립트랑 프로세스 관리 실습". query에는 배우고 싶은 개념·키워드를 자연어로 넣는다.
    과목명·교수명, 또는 "리눅스 수업", "네트워크 관련 강의", "모바일 과목"처럼 한 단어로 과목
    목록을 찾을 때는 search_courses를 쓴다 — 그 단어가 과목명에 들어 있으면 search_courses가
    빠짐없이 찾아 주지만, 이 Tool은 유사도 상위 3개만 반환해 실제 과목이 누락되거나 관련 없는
    과목이 섞인다. 각 결과에는 course_code, 과목명, 교수, 분반코드 목록, 개요 한두 줄, relevance(0~1
